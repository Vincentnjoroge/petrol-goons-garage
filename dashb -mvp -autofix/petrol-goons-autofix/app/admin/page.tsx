...
export default function SuperAdminPage() {
  ...
  const handleGarageStatus = async (garage: Garage, newStatus: 'approved' | 'suspended') => {
    ...
    try {
      // Set status to 'active' (not just 'approved') — dashboard checks for both
      const dbStatus = newStatus === 'approved' ? 'active' : newStatus
      await updateGarageProfile(garage.id, { status: dbStatus })
      setGarages(prev => prev.map(g => g.id === garage.id ? { ...g, status: dbStatus } : g))

      // FIX: When approving, also heal the owner's user doc.
      // If the garage was submitted before the operation-order fix, the owner's
      // user doc may still have role:'customer' and no garageId. This repairs it.
      if (newStatus === 'approved' && garage.ownerId) {
        try {
          const { doc, getDoc, updateDoc, setDoc, serverTimestamp } = await import('firebase/firestore')
          const { db } = await import('@/lib/firebase')
          const userRef = doc(db, 'users', garage.ownerId)
          const userSnap = await getDoc(userRef)
          if (userSnap.exists()) {
            const userData = userSnap.data()
            // Only update if they're not already garage_owner for this garage
            if (userData.role !== 'garage_owner' || userData.garageId !== garage.id) {
            ...
            await setDoc(userRef, {
              ...
              displayName: garage.ownerName || '',
              phoneNumber: garage.ownerPhone || '',
              role: 'garage_owner',
              garageId: garage.id,
          ...
          console.error('Failed to update owner user doc:', userErr)
          // Non-fatal — garage is approved, user doc can be fixed manually
        }
      }
